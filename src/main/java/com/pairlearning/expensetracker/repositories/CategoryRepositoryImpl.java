...
public class CategoryRepositoryImpl implements CategoryRepository {
    ...
    private static final String SQL_FIND_BY_ID = "SELECT C.CATEGORY_ID, C.USER_ID, C.TITLE, C.DESCRIPTION, " + "COALESCE(SUM(T.AMOUNT), 0) TOTAL_EXPENSE " + "FROM ET+TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID = T.CATEGORY " + "WHERE C.USER_ID = ? AND C.CATEGORY_ID = ? GROUP BY C.CATEGORY_ID";
    ...
    public Integer create(Integer userId, String title, String description) throws EtBadRequestException {
        try {
            ...
            throw new EtBadRequestException("Invalid Amount");
        }
    }
